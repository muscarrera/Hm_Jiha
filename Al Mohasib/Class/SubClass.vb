...
Public Class SubClass
    ...
    Public Sub FillGroupes()
        ...
        If Form1.txtGroupe.Text.Contains("4") = False Then Exit Sub
    ...
    Public Sub FillGrStock()
        ...
        Using c As DataAccess = New DataAccess(My.Settings.ALMohassinDBConnectionString)
            ...
            Dim name As String = c.SelectByScalar("Depot", "name", params)

            If name <> "HAMMAM" Then
                c.DeleteRecords("Depot")
                params.Clear()
                params.Add("dpid", 1)
                params.Add("name", "HAMMAM")
                c.InsertRecord("Depot", params)

                params.Clear()
                params.Add("dpid", 2)
                params.Add("name", "COIFFURE")
                c.InsertRecord("Depot", params)

                params.Clear()
                params.Add("dpid", 3)
                params.Add("name", "TOURKI")
                c.InsertRecord("Depot", params)

                params.Clear()
                params.Add("dpid", 4)
                params.Add("name", "PRODUIT")
                c.InsertRecord("Depot", params)
        ...
        Form1.FLPStock.Controls.Clear()

        For i As Integer = 1 To 4

            If Form1.txtGroupe.Text.Contains(i) = False Then Continue For
            Dim STR As String() = {"NONE", "HAMMAM", "COIFFURE", "TOURKI", "PRODUIT"}

            Dim bt As New Button
            Dim rnd As New Random
            Dim cr As Color = Color.FromArgb(rnd.Next(0, 255), rnd.Next(0, 255), rnd.Next(0, 255), rnd.Next(0, 255))
        ...
        Try
            ...
            Else
                For i As Integer = 0 To artdt.Rows.Count - 1
                    ...
                    Dim ccc = artdt.Rows(i).Item("cid").ToString
                    If CInt(ccc) > 4 Then ccc = 4

                    If Form1.txtGroupe.Text.Contains(ccc) = False Then Continue For
    ...
    Public Sub ctg_click(ByVal sender As System.Object, ByVal e As EventArgs)
        ...
        Try
            Dim artta As New ALMohassinDBDataSetTableAdapters.ArticleTableAdapter
            Dim artdt = artta.GetDataBycid(CInt(bt2.Tag))
        ...
        End Try
    End Sub
    Public Sub ctg_click_HAMMAM(ByVal sender As System.Object, ByVal e As EventArgs)

        Dim bt2 As Button = sender
        Form1.FlowLayoutPanel1.Controls.Clear()
        Try
            Dim cid = bt2.Tag.ToString.Split("-")(0)
            Dim code = bt2.Tag.ToString.Split("-")(1)

            Dim artta As New ALMohassinDBDataSetTableAdapters.ArticleTableAdapter
            Dim artdt = artta.GetDataByCidHammam(CInt(cid), code)
        ...
        End Try
    End Sub
    Public Sub FillGroupesHammam(ByVal cid As Integer)
        'Exit Sub
        Form1.FlowLayoutPanel1.Controls.Clear()
        Dim msg

        Try
            msg = My.Computer.Registry.GetValue("HKEY_LOCAL_MACHINE\SOFTWARE\AlMohassib", "Cat_Ham_" & cid, Nothing)
            If msg = Nothing Then
                Dim bt As New Button
                bt.Tag = cid & "- "
                ctg_click_HAMMAM(bt, Nothing)
                Exit Sub
            End If
        Catch ex As Exception
            Dim bt As New Button
        ...
        For i As Integer = 0 To cat.Length - 1
            ...
            Form1.FlowLayoutPanel1.Controls.Add(bt)
            bt.Dock = DockStyle.Top
            AddHandler bt.Click, AddressOf ctg_click_HAMMAM
        Next
    ...
    Private Sub art_click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If isAdminFacture(Form1.RPl.FctId) Then
            fillFactures(Form1.btswitsh.Tag)
    ...
    Private Sub EditArticle(ByVal R As ALMohassinDBDataSet.ArticleRow)
        ...
        Try
            art.txtCatHammam.text = CInt(R.elements)
        Catch ex As Exception
            art.txtCatHammam.text = "0"
        End Try
    ...
    Public Sub NewFacture(ByVal clid As String, ByVal cn As String, ByVal cad As String, ByVal nm As String)
        Try
            ...
            Using c As DataAccess = New DataAccess(My.Settings.ALMohassinDBConnectionString)
                ...
                params.Add("total", 0)
                params.Add("avance", 0)
                params.Add("date", Format(dte, "dd/MM/yyyy HH:mm:ss"))
                params.Add("admin", False)
                params.Add("writer", CStr(Form1.adminName))
                params.Add("tp", tp)
                params.Add("payed", False)
    ...
    End Sub
    Public Sub UpdateClient(ByVal fid As Integer, ByVal isS As Boolean)
        Dim chs As New ChoseClient
        chs.isSell = isS
        If chs.ShowDialog = Windows.Forms.DialogResult.OK Then
